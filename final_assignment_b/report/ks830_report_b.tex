...
With a view to get a more generalisable model, I split the data into qualitatively different sets using a single-component t-SNE projection (at perplexity 30), holding out the top decile of values for testing, and another decile for validation.
...
I wish to be able to predict the length of the follow-on visit for a patient using the data of just one encounter, since the original dataset suggests that in majority (54\%) of cases this is indeed the most information that a hospital would have when trying to efficiently manage its resources. For this task I implement a \textit{feedforward neural network} architecture.
...
MAIN FUCKING INSIGHT THE SIMPLER U GO THE BETTER FFS

\paragraph{Interpretation}
TODO if results continue to be sh*t, then: current stay features are not enough to predict the future length of stay. Medical emergencies are unpredictable in nature. 

TODO Perhaps patients with 
