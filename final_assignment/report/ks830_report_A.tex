...
Prediction of three readmission outcome labels is an instance of a multi-class classification task. I implement the multi-class \textit{Naive Bayes} classifier, \textit{stochastic gradient descent} and \textit{logistic regression} classifiers (using \textit{one-vs-all} strategy where appropriate).

5-fold cross-validation gave the best mean accuracy results for \texttt{LogisticRegression} classifier with mean accuracy of 50.4\%. All three classifiers performed better than the baseline mean accuracy (random guessing based on label distribution) of 34.2\% (Table \ref{multiclass}). 

\begin{table}[]
	\begin{tabularx}{\linewidth}{XXXX}
		\hline
		\textbf{Baseline} & \textbf{NB} & \textbf{Log. reg.} & \textbf{SGD} \\ \hline
		34.2\%            & 38.0\%               & 50.4\%                       & 48.2\%                 \\ \hline
		\end{tabularx}
\caption{Mean cross-validation accuracies for selected multi-class classifiers.}\label{multiclass}
\end{table}
