...
\paragraph{Feature sets} I will analyse two feature sets: \textit{full} feature set with 192 total features (after preprocessing), and \textit{reduced} feature set with 87 features, which excludes the medication name data and medical specialty. 
...
Prediction of three readmission outcome labels is an instance of a multi-class classification task. I implement the multi-class \textit{Naive Bayes}, \textit{stochastic gradient descent} and \textit{logistic regression} classifiers (using \textit{one-vs-all} strategy where appropriate).

The results are generally better for the reduced feature set, where 5-fold cross-validation gave the best results for \texttt{LogisticRegression} classifier (mean accuracy 53.4\%). All three classifiers performed better than the baseline mean accuracy (random guessing based on label distribution) of 33.3\% (Table \ref{multiclass}). 

\begin{table}[]
	\begin{tabularx}{\linewidth}{XXXXX}
		\hline
								 & \textbf{Baseline} & \textbf{NB} & \textbf{LogReg} & \textbf{SGD} \\ \hline
		full   & 33.3\%            & 37.1\%      & 53.3\%          & 49.2\%       \\
		reduced & 33.3\%            & 38.5\%      & 53.6\%          & 52.2\%       \\ \hline
		\end{tabularx}
\caption{Mean cross-validation accuracies for selected multi-class classifiers and feature sets.}\label{multiclass}
\end{table}
