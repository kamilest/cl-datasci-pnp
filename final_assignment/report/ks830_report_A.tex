...
\paragraph{Precision, recall and $F_1$ score} I report the scores for each class individually, combined using macro-averaging (which treats all classes equally) by averaging the metrics computed for each class.

Scores for  each class (precision, recall, F1):
...
Figure \ref{rocauc} shows the receiver operating characteristic curves.

\begin{figure}[htb!]
	\centering
	\includegraphics[width=0.6\linewidth]{figures/confmatrix.png}
	\caption{Confusion matrix for the best classifier.}\label{confmatrix}
\end{figure}

\begin{figure}[htb!]
	\centering
	\includegraphics[width=\linewidth]{figures/rocauc.png}
	\caption{ROC-AUC curve for the best classifier.}\label{rocauc}
\end{figure}
